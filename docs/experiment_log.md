...
#### 发现 6（已推翻）：~~MoE 本身也有害~~ → MoE 收敛慢但最终效果好

早期数据（27K 步）显示 MoE 全面落后，但训练继续后结果逆转：

**早期（27K 步，过早下结论）：**

| 任务 | MoE-only (27K) | Baseline2 (74K) |
|------|---------------|-----------------|
| reach | 1704 | 4828 |
| door | 382 | 3863 |
| pick-place | 5.6 | 8.5 |
...
| 任务 | MoE-only (步数) | Baseline2 (步数) | 结论 |
|------|----------------|-----------------|------|
| reach | 4835 (349K) | 4841 (981K) | **持平，MoE 用 1/3 步数** |
| push | 2395 (351K) | 4642 (967K) | MoE 落后，但步数差 3x |
| door-open | **4539 (97K)** | 4392 (147K) | **MoE 更优，步数更少** |
| pick-place | **957 (350K)** | 8.76 (966K) | **MoE 碾压，957 vs 9** |

**关键发现**：
- MoE 训练速度慢 3x（fps 10 vs 27），但**样本效率更高**
- 在最难的 pick-place 上，MoE-only 957 vs Baseline2 8.76 — 巨大优势
- 在 door-open 上，MoE 用更少步数达到更高分
- **之前"MoE 有害"的结论是错误的，下结论太早**

**注意**：Baseline2 pick-place 只有 8.76（上轮 Baseline 是 3178），可能因为
新配置中 imag_length=20（上轮是默认 15）。更长 imagination 可能对原版 DreamerV3
不利，但 MoE 能更好地处理长 rollout。需要进一步验证。

#### 发现 7（修正）：组件评估更新

| 组件 | 预期效果 | 实际效果 | 状态 |
|------|---------|---------|------|
| MoE Dynamics | 多专家建模不同动力学 | **有效！收敛慢但样本效率高** | ✅ 验证正面 |
| TRD 信任加权 | 降权不可信想象 | **有害** | ✗ 确认负面 |
| DAgger | 主动纠正不准区域 | 未验证 | 待测 |
| 优先级采样 | 聚焦不准区域训练 | 未验证 | 待测 |

### 反思

1. **过早下结论是危险的**：MoE 在 27K 步时看起来很差，但 350K 步时逆转。
   RL 实验必须跑到足够步数才能下结论
2. **TRD 信任加权确认有害**：第二轮实验中 Grounded (MoE+TRD) 全面落后 Baseline，
   但 MoE-only 反而不错 → 问题在 TRD 加权而非 MoE
3. **imag_length 影响待查**：Baseline2 pick-place 从上轮 3178 暴跌到 8.76，
   可能是 imag_length 从 15 改到 20 导致

### 后续方向

**明确的下一步**：
1. MoE 保留 — 已验证在 pick-place 上有显著提升
2. TRD 加权去掉 — 已确认有害
3. TRD 改为 reward penalty（MOPO 方式）— 待测试
4. 验证 imag_length 影响 — 跑 Baseline3 用 imag_length=15 对比
5. 在 MoE-only 基础上加 DAgger — MoE 找到更好的动力学后，DAgger 纠正可能有效

**最有前景的方案**：MoE + reward penalty（去掉信任加权）+ DAgger
